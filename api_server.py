...
class AIConfirmer:
    """
    LLM-only confirmation policy (fail-closed).
      - All provider keys, URLs, and models are read from .env only.
      - Provider priority chain: DeepSeek -> OpenAI -> Groq
      - local_precheck is NON-AUTHORITATIVE validation only (never sets approved=True).
      - If use_llm is False or all providers fail, approved=False (no fallback).
    """
    ...
    def confirm(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Precheck for basic validation only (NOT approval)
        ...
        # Try provider priority chain: DeepSeek -> OpenAI -> Groq
        ...
        if ok_llm and isinstance(llm_out, dict):
            ...
            out["provider"] = llm_out.get("provider", "unknown")
    ...
    def _llm_confirm_with_priority(self, payload: Dict[str, Any], ai_cfg: Dict[str, Any]) -> Tuple[bool, Dict[str, Any], List[Dict]]:
        """
        Priority chain: DeepSeek -> OpenAI -> Groq (all OpenAI-compatible).
        All keys, URLs, and model names are read from environment variables only.

        Returns: (success, response, provider_trace)
        provider_trace = [{"provider": "deepseek", "status": "timeout", "reason": "..."}]
        """
        providers = [
            {
                "name": "deepseek",
                "url": os.environ.get("DEEPSEEK_API_URL", "").strip(),
                "key_env": "DEEPSEEK_API_KEY",
                "model": os.environ.get("DEEPSEEK_MODEL", "").strip(),
            },
            {
                "name": "openai",
                "url": os.environ.get("OPENAI_API_URL", "").strip(),
                "key_env": "OPENAI_API_KEY",
                "model": os.environ.get("OPENAI_MODEL", "").strip(),
            },
            {
                "name": "groq",
                "url": os.environ.get("GROQ_API_URL", "").strip(),
        ...
        for p in providers:
            ...
            ok, response = self._llm_http_chat_completions(
                url=url,
                api_key=key,
                model=model,
                system_text="You are a strict risk manager. Return JSON only. No markdown.",
                user_text=self._build_llm_prompt(payload, ai_cfg),
                temperature=float(ai_cfg.get("llm_temperature", 0.1)),
                max_tokens=int(ai_cfg.get("llm_max_tokens", 220)),
                timeout_sec=float(ai_cfg.get("llm_timeout_sec", 8.0)),
            )

            if ok:
                ok_json, parsed = self._extract_llm_json(response)
                if ok_json:
                    provider_trace.append({
                        "provider": p["name"],
                        "status": "success",
                    ...
                    parsed = self._normalize_llm_parsed(parsed)
                    parsed["provider"] = p["name"]
                    parsed["model"] = model
                    return True, parsed, provider_trace
                else:
                    provider_trace.append({
                        "provider": p["name"],
                        "status": "failed",
    ...
    @staticmethod
    def _normalize_llm_parsed(out: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize LLM JSON output: confidence, reason, bullets."""
        ...
        try:
            ...
            out["confidence"] = max(0.0, min(1.0, c))  # Clamp confidence to [0.0, 1.0]
        except Exception:
            out["confidence"] = 0.0
